        ...
        <div className="max-w-7xl mx-auto w-full relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-space-32">
            {INSIGHTS_DATA.map((art, idx) => (
              <div 
                key={art.id}
                className="group flex flex-col gap-space-16 border border-charcoal/10 p-space-20 bg-stone hover:border-bronze/40 transition-all duration-700 rounded-sm shadow-md"
              >
                <div className="w-full aspect-[4/3] overflow-hidden border border-charcoal/10 bg-concrete relative">
                  <img 
                    src={art.image} 
                    alt={art.title} 
                ...
                <div className="flex flex-col gap-space-12 mt-2">
                  ...
                  <h3 className="font-display text-xl font-light text-charcoal group-hover:text-bronze transition-colors leading-snug">
                    {art.title}
                  </h3>
                ...
                <div className="border-t border-charcoal/10 pt-4 mt-4 text-right">
                  <span className="font-mono text-[8px] text-bronze hover:text-charcoal transition-colors">READ BRIEFING →</span>
                </div>
              </div>
            ))}
          </div>
