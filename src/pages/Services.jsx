    ...
    <div className="w-full flex flex-col bg-charcoal text-ivory selection:bg-bronze selection:text-charcoal pt-0 select-none">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-space-32">
          {SERVICES_DATA.map((srv, idx) => (
            <Link 
              key={srv.id}
              to={`/services/${srv.id}`}
              className="group border border-white/10 p-space-32 bg-graphite rounded-sm hover:border-bronze/45 transition-all duration-700 flex flex-col justify-between min-h-[360px] shadow-2xl relative"
            >
              <div className="absolute top-0 right-0 border-b border-l border-white/10 p-space-12 h-label-mono text-[8px]">
                CODE // 0{idx+1}
              </div>

              <div className="flex flex-col gap-space-16 mt-4">
                <span className="font-mono text-[8.5px] text-bronze font-bold">{srv.category.toUpperCase()}</span>
              ...
              <div className="border-t border-white/10 pt-space-24 mt-space-24 flex justify-between items-center text-xs">
                <span className="font-mono text-stone text-[9px] uppercase tracking-wider">READ COMPLIANCE</span>
                <span className="font-mono text-[9px] text-bronze hover:text-white transition-colors">→</span>
              </div>
            </Link>
          ))}
        </div>
      ...
      <section className="py-space-96 px-space-24 md:px-space-40 bg-graphite border-t border-white/5 relative">
        ...
        <div className="max-w-4xl mx-auto text-center flex flex-col gap-space-24 relative z-10">
          <span className="h-label-mono text-bronze">[STRUCTURAL CONSULTATIVE REGISTRY]</span>
